...
```bash
git update-index --skip-worktree docs/guide-curateurs.md
```

Pour vérifier l'état :

```bash
git ls-files -v docs/guide-curateurs.md   # affiche 'S' si skip-worktree est actif
```

Pour réactiver le tracking (ex. pour mettre à jour le placeholder) :

```bash
git update-index --no-skip-worktree docs/guide-curateurs.md
```
