...
echo "[$START_TS] Starting Infoscience pipeline (365-day sliding window)..."

# Exécution principale : fenêtre glissante de 365 jours, log + tee
"$PYTHON_BIN" data_pipeline/main.py --source zenodo --window-days 365 -v 2>&1 | tee -a "$LOG_DIR/cron.out"
