...
echo "[$START_TS] Starting Infoscience pipeline (20-day sliding window)..."

# Exécution principale : fenêtre glissante de 20 jours, log + tee
"$PYTHON_BIN" data_pipeline/main.py --source scopus,openalex,crossref --window-days 20 -v 2>&1 | tee -a "$LOG_DIR/cron.out"
