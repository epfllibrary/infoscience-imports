...
def _():
    ...
    from data_pipeline.harvester import ScopusHarvester, WosHarvester, CrossrefHarvester, OpenAlexCrossrefHarvester, ZenodoHarvester
    ...
    form = mo.md('''
        ...
        **Define query by source**
        ...
        sources=mo.ui.multiselect(
            label="Select Source:",
            options=["Scopus", "WoS", "Crossref", "OpenAlex", "Zenodo"]
        ),
        ...
        zenodo_query=mo.ui.text_area(
            label="Zenodo query:",
            value=default_queries["zenodo"],
            placeholder="Default Zenodo query",
            rows=2,
            full_width=True,
...
def _(
    ...
    scopus_df = wos_df = crossref_df = openalex_df = zenodo_df = pd.DataFrame()
    ...
    if "Zenodo" in sources:
        zenodo_df = pd.DataFrame(
            ZenodoHarvester(start_date, end_date, form.value["zenodo_query"]).harvest()
        )
    return crossref_df, openalex_df, scopus_df, wos_df, zenodo_df


@app.cell
def _(crossref_df, mo, openalex_df, scopus_df, wos_df, zenodo_df):
    harvested_results = []
    ...
    if not zenodo_df.empty:
        harvested_results.append(mo.md("#### Zenodo Results"))
        harvested_results.append(mo.ui.table(zenodo_df))
...
def _(
    ...
    dataframes = [df for df in [scopus_df, wos_df, crossref_df, openalex_df, zenodo_df] if not df.empty]
